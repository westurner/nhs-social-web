...
def list_animals(request, category):
    ...
    return object_list(request, db.Query(Animal).filter('category =',category).filter('status =','adoptable').filter('last_checked >', datetime.date.today()-datetime.timedelta(5)).order('-last_checked').order('-brought_to_shelter'), paginate_by=15, extra_context={'page_title':ANIMAL_CATEGORIES[category],'category':category})
